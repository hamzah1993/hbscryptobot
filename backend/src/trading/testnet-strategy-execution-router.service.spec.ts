...
describe('TestnetStrategyExecutionRouterService', () => {
  ...
  function serviceFor(exchange: 'BINANCE' | 'BYBIT' | 'OKX') {
    ...
    const binance = { executeMarketOrder: jest.fn().mockResolvedValue({ exchange: 'BINANCE', ok: true }) } as any;
    return { service: new TestnetStrategyExecutionRouterService(prisma, binance), binance };
  }
  ...
  it('routes a verified Binance Testnet strategy through the established execution service', async () => {
    ...
    await expect(service.executeMarketOrder('user-1', input)).resolves.toEqual({ exchange: 'BINANCE', ok: true });
